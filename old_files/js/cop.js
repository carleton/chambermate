...
//Validate experiment title and the different form fields
...
//Set up COP inputs after validating form fields
function setUpCOP() {
  if (COPValidate()) {
    //If fields are valid
...
function obj_one_ClockRunning() {
  var currentTime = new Date(),
    timeElapsed = new Date(
      currentTime - obj_one_TimeBegan - obj_one_StoppedDuration
    ),
...
function obj_two_ClockRunning() {
  var currentTime = new Date(),
    timeElapsed = new Date(
      currentTime - obj_two_TimeBegan - obj_two_StoppedDuration
    ),
    hour = timeElapsed.getUTCHours(),
    min = timeElapsed.getUTCMinutes(),
    sec = timeElapsed.getUTCSeconds(),
...
function clockRunningCop() {
  ...
  if ((timer >= 590 && timer <= 600) || (timer >= 890 && timer <= 900)) {
  ...
  while (j <= copBehavior.length) {
    while (i <= columns.length) {
      ...
      if (columns[i] == copBehavior[j]['stim']) {
        ...
        if (
          copBehavior[j]['stim'] == 'center' &&
          copBehavior[j - 1]['stim'] == 'inTwo'
        ) {
